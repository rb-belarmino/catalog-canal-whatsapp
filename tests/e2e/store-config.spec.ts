...
test.describe('User Story 5: Store WhatsApp Number Configuration', () => {
  test('updates store name and phone number in admin settings', async ({
    page
  }) => {
    ...
    await expect(
      page.getByText(/Configurações salvas com sucesso/i)
    ).toBeVisible()

    // Verify change reflected in public catalog
    await page.goto('/')
    await expect(
      page.getByRole('link', { name: /Canal Concept/i })
    ).toBeVisible()
  })
})
