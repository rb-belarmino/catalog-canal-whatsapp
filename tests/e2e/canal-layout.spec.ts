...
test.describe('Canal Concept Layout & Flow E2E Tests', () => {
  test('displays Canal Concept branding and clean header without top announcement bar', async ({
    ...
    const announcementBar = page.getByRole('region', {
      name: /Avisos e promoções/i
    })
  ...
  test('filters product list instantly using the search bar', async ({
    page
  }) => {
    await page.goto('/')
  ...
  test('manages Lista de Desejos and generates formatted WhatsApp link', async ({
    ...
    const checkoutBtn = page.getByRole('button', {
      name: /Envie aqui a sua lista|Enviar Lista no WhatsApp/i
    })
