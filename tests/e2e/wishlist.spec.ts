...
test.describe("User Story 3: Wishlist Flow", () => {
  ...
  test("allows adding and removing item directly from product card", async ({ page }) => {
    ...
    const firstAddButton = page.locator('button:has-text("Adicionar aos Desejos")').first();
    ...
    if (hasProducts) {
      ...
      const addedButton = page.locator('button:has-text("Na Lista de Desejos")').first();
      await expect(addedButton).toBeVisible();

      // Wishlist floating button badge should show 1
      const floatingBtn = page.locator('[data-testid="floating-wishlist-button"]');
