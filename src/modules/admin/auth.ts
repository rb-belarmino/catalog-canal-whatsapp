...
export function verifySessionToken(token?: string | null): boolean {
  ...
  try {
    const secret = getSecret();
    const expectedHmac = crypto.createHmac("sha256", secret).update(issuedAt).digest("hex");
    ...
    logger.error("AdminAuth", "Error verifying session token", { err: String(err) });
    return false;
  }
}
...
export function verifyAdminPassword(inputPassword: string): boolean {
  try {
    ...
    logger.error("AdminAuth", "Error verifying admin password", { err: String(err) });
    return false;
  }
}
