...
export async function createProductAction(input: {
  ...
  try {
    ...
    logger.error('AdminActions', 'Error creating product', {
      error: String(err)
    })
...
export async function updateProductAction(input: {
  ...
  try {
    ...
    logger.error('AdminActions', 'Error updating product', {
      error: String(err)
    })
...
export async function deleteProductAction(id: string): Promise<ActionResult> {
  try {
    ...
    logger.error('AdminActions', 'Error deleting product', {
      error: String(err)
    })
...
export async function reorderProductsAction(
  ...
  try {
    ...
    logger.info('AdminActions', 'Products reordered', {
    ...
    logger.error('AdminActions', 'Error reordering products', {
      error: String(err)
    })
  ...
  try {
      ...
      if (cleanPhone) {
        ...
        if (!/^[1-9][0-9]{9,14}$/.test(cleanPhone)) {
          return {
            success: false,
            error:
              'Número de WhatsApp inválido. Digite apenas DDD + número (ex: 11 99999-8888).'
          }
        }
      }
    ...
    if (input.topAnnouncement !== undefined) {
      ...
      if (trimmed.length > 255) {
        return {
          success: false,
          error:
            'O comunicado da barra de topo deve ter no máximo 255 caracteres.'
        }
      }
    ...
    await prisma.shopConfig.upsert({
      ...
      create: {
        ...
        topAnnouncement:
          data.topAnnouncement ?? 'PARCELE EM ATÉ 10X SEM JUROS | 5% OFF NO PIX'
      },
      update: data
    })
    ...
    logger.error('AdminActions', 'Error updating shop config', {
      error: String(err)
    })
    return {
      success: false,
